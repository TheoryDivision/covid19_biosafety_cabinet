...
# Make plot
ggplot(data=uv_meter_data) + geom_raster(aes(x=x,y=y,fill=radiation)) + theme_classic() + scale_fill_continuous(expression(atop("UV radiation",~(paste(mu,W)/cm^2))), type = "viridis")  + facet_wrap(~hood) + theme(axis.title = element_blank(), axis.text = element_blank(), axis.ticks = element_blank(), axis.line = element_blank())
