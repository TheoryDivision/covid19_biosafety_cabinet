...
df %>% group_by(hood, x, y) %>% summarise(min=min(mean-off_mean), max=max(mean-off_mean), ratio=max(mean-off_mean)/min(mean-off_mean))

# Calculate summary statistics across entire dataset
mean(df$mean - df$off_mean)
min(df$mean - df$off_mean)
max(df$mean - df$off_mean)
sd(df$mean - df$off_mean)
```
